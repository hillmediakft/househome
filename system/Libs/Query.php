...
//				private $joinTypes = array('left', 'inner', 'cross', 'right', 'natural', 'straight');
	 ...
	 * @return void or false
	 */
	public function set_having($column, $oper = null, $data = null, $type = 'and')
	{
	//első parameter string és van operátor, és van data, ami string, vagy integer tipusú
		if(is_string($column) && !is_null($oper) && (is_string($data) || is_int($data))) {
			$string = strtoupper($type) . ' ' . $column . ' ' . $oper . ' ' . "?";
	 ...
	 * @return void
	 */
	public function set_join($type, $table, $first = null, $oper = null, $second = null)
	{
		if (is_array($table)) {
			$table = '(' . implode(',', $table) . ')';
		}

		if(!is_null($first) && is_null($oper) && is_null($second)){
			$this->joins[] = strtoupper($type) . ' JOIN ' . $table . ' ON ' . $first;
		}
		else if(!is_null($first) && !is_null($oper) && !is_null($second)) {
			$this->joins[] = strtoupper($type) . ' JOIN ' . $this->add_backtick($table) . ' ON ' . $this->add_backtick($first) . ' ' . $oper . ' ' . $this->add_backtick($second);
		}
		else if(is_null($first) && is_null($oper) && is_null($second)) {
			$this->joins[] = strtoupper($type) . ' JOIN ' . $this->add_backtick($table);
		}
	}
...
//--------------------------------------------------------------------------------------
	...
	public function select()
	{
		...
		$sql = "SELECT " . $this->columns . " FROM " . $tablename . $this->getJoins() . $this->getWhere() . $this->getGroupby() . $this->getHaving() . $this->getOrderby() . $this->getLimit();
	...
	/**
	 * Visszadja a HAVING feltételek stringjét
	 *
	 * @access private
	 * @return String
	 */
	private function getHaving()
	{
		...
		// Implode where pecices then remove the first AND or OR
		return " HAVING" . ltrim( implode(" ", $this->having), "ANDOR" );
	}
