                    ...
                    <div style="overflow: hidden;">
                        <a href="<?php echo $this->request->get_uri('site_url') . Config::get('url.ingatlanok.adatlap.' . LANG) . '/' . $ingatlan['id'] . '/' . $this->str_helper->stringToSlug($ingatlan['ingatlan_nev_' . LANG]); ?>">
                            <?php if ($ingatlan['kepek']) { ?>
                                <img style="height:55px; width:auto;" src="<?php echo $this->url_helper->thumbPath($this->getConfig('ingatlan_photo.upload_path') . $photo_array[0], null, 'small'); ?>" alt="<?php echo $ingatlan['ingatlan_nev_' . LANG]; ?>">
                            <?php } else { ?>
                                <img style="height:55px; width:auto;" src="<?php echo $this->getConfig('ingatlan_photo.upload_path') . 'placeholder.jpg'; ?>" alt="<?php echo $ingatlan['ingatlan_nev_' . LANG]; ?>">
    <?php } ?>
                        </a>
                    </div>
                </div>
